...
class Client:
    ...
    def __sameHostNameConnected(self,nick,ipaddress,status,clients):
        result = filter(
            lambda host: host["nick"]==nick and host["ipaddress"]!=ipaddress and host["status"]==status,
            clients
        )
        return True if len(list(result))>0 else False

    # if the new client his ip ipaddress or nick is in the clients list
    def __differentHostAddress(self,ip,nick,clients):
        if clients==None:
            return True
        result = filter(lambda host: host["ipaddress"]==ip or host["nick"]==nick,clients)
        return True if len(list(result))==0 else False

    # this method adds or update clients
    # - This adds clients when these have ip ipaddress and nick new (not is in the list)
    # - This adds clients when the clients list is empty
    # - This update the clients when:
    #      - The client changes the ip ipaddress and use the same nick
    #      - The client changes the nick but the host has the same ip ipaddress
    ...
    def registre(self,nick,status):
        ...
        client = Requests("services","GET",f"https://classadmin.server/api/clients/{self.ipaddress[0]}/{nick}").run().json()["result"]
        ...
        if (client==None and status=="CONNECTED") or (self.__differentHostAddress(self.ipaddress[0],nick,clients) and status=="CONNECTED"):
        ...
        elif self.__sameHostNameConnected(nick,self.ipaddress[0],"CONNECTED",clients):
            return "sameUser"

        # update the client
        else:
            cLientsUpdate = Requests("services","GET",f"https://classadmin.server/api/clients/{self.ipaddress[0]}/{nick}").run().json()["result"]
            ...
            Requests("services","PUT",f"https://classadmin.server/api/clients/{self.ipaddress[0]}/{nick}",{
                    "nick":nick,
                    "ipaddress":self.ipaddress[0],
                    "port":self.ipaddress[1],
                    "status":status,
